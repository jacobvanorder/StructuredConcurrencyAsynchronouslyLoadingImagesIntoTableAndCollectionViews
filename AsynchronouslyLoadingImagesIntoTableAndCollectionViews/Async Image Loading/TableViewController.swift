...
class TableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        dataSource = UITableViewDiffableDataSource<Section, Item>(tableView: tableView) {
            ...
            if let itemImage = item.image { // Did we set the image previously?
            ...
            } else if let cachedImage = ImageCacheActor.publicCache.image(named: item.imageName) { // Did the actor already load this image elsewhere?
                ...
                Task { [weak self] in
                    ...
                    do {
                        newImage = try await ImageCacheActor.publicCache.load(imageNamed: item.imageName)
                    } catch {
